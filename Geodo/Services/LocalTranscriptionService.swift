...
class LocalTranscriptionService: TranscriptionService {
    ...
    func transcribeWithTimestamps(audioURL: URL, model: any TranscriptionModel) async throws -> [WhisperTimestampedSegment] {
        guard model.provider == .local else {
            throw WhisperStateError.modelLoadFailed
        }

        logger.notice("Initiating local transcription with timestamps for model: \(model.displayName)")
        ...
        let segments = await whisperContext.getTranscriptionWithTimestamps()

        logger.notice("✅ Local transcription with timestamps completed: \(segments.count) segments")
